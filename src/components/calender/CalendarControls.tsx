  ...
  const handlePreviousClick = useCallback(() => {
    if (navView === 'years') {
      setDisplayYears((prev) => ({
        from:
          prev.from - (prev.to - prev.from + 1) < 0
            ? 0
            : prev.from - (prev.to - prev.from + 1),
...
const CalenderContent = ({
  ...
  const currentMonth =
    MONTHS[selectedMonth?.getMonth() ?? new Date().getMonth()];
  ...
  if (navView === 'years') {
    ...
    return (
      <div className={cn('grid grid-cols-3 gap-y-2', className)} {...props}>
        {MONTHS.map((month, i) => {
          const isBefore =
            differenceInCalendarDays(new Date(currentYear, i, 1), startMonth!) <
            0;
          const isAfter =
            differenceInCalendarDays(
